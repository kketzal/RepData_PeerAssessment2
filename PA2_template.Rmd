...
# We use the factor array index to access the factor values and change them...
...
# Preview
my_economic_summary_propdmg
my_economic_summary_cropdmg
my_economic_total_summary
...
# p <- p + ggtitle("Histogram FATALITIES")
x <- my_health_summary_fatalities[1:20, ]


x$EVTYPE <- reorder(x$EVTYPE, x$SUM_FATALITIES)

# Create a second plot for the new dataset with imputing missing values
p2 <- ggplot(x, aes(x=EVTYPE, y=SUM_FATALITIES, fill=SUM_FATALITIES))
p2 <- p2 + geom_histogram(stat='identity') + coord_flip()
p2 <- p2 + scale_fill_gradient("Nº Fatalities", 
                             low = "green", 
                             high = "red")
...
p2 <- p2 + theme(text = element_text(size=10)) 
 ...
 p2 <- p2 + ylab("Total Fatalities")
 p2 <- p2 + ggtitle("Histogram - Total Fatalities By Event Type")
